...
export const ContainerPlan = ({ children, showPanel = false }: { children: React.ReactNode, showPanel: boolean }) => {
  return (
    <div className={`grid ${showPanel ? "grid-cols-2 max-sm:grid-cols-1" : "grid-cols-1"}
             gap-4 max-sm:grid-cols-1 transition-all
             max-h-[90vh] overflow-hidden max-sm:*overflow-y-scroll max-sm:max-h-full duration-300 ease-in-out`}>
      {children}
    </div>
  )
}
