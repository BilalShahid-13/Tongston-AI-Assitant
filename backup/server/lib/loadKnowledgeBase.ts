...
import { isDriveFolderLink, isDriveFileLink, getGoogleDriveDirectDownload } from '../utils/fetchFolderId';
...
async function extractPdfContent(link: string): Promise<string | null> {
  try {
...
async function extractFolderContent(folderLink: string): Promise<(string | null)[]> {
  try {
    const folderIdMatch = folderLink.match(/\/folders\/([a-zA-Z0-9_-]+)/);
    if (!folderIdMatch) throw new Error('Invalid Google Drive folder link');
...
async function loadKnowledgeBaseFromExcel(filePath: string) {
  try {
    // Read the Excel file
    ...
    for (const sheetName of sheetNames) {
      ...
      for (const row of data) {
        ...
        const curriculumMapping = typedRow['Curriculum Mapping']
          ? typedRow['Curriculum Mapping'].split(',').map((item: string) => item.trim())
          : [];
        ...
        const enrichedResources = await Promise.all(
          resources.map(async (resource: any) => {
            ...
            if (resource.link) {
              ...
              if (isDriveFolderLink(resource.link)) {
                const folderContents = await extractFolderContent(resource.link);
                content = folderContents.filter((c) => c !== null).join(' ') || null;
              } else if (isDriveFileLink(resource.link)) {
                const fileUrl = getGoogleDriveDirectDownload(resource.link);
...
async function main() {
  try {
    ...
    const excelFilePath = path.join(__dirname, '../public/AI Chatbot (K12) Knowledge base sort sheet.xlsx');
