...
function studentConductCharacterPlanAssessmentPrompt(context) {
    ...
    const continuousAssessmentWeek = Array.isArray(context.continuousAssessmentWeek) ? context.continuousAssessmentWeek : [];
    ...
    const teachingAids = Array.isArray(context.teachingAids) ? context.teachingAids : [];
    const classSize = context.classSize;
    const timeAvailable = context.timeAvailable;
    const noQuestions = context.noQuestions;
    const questionTypes = Array.isArray(context.questionTypes) ? context.questionTypes : [];
    // Optional fields
    const state = context.state ?? '';
    const cities = context.cities ?? '';
    const mission = context.mission ?? 'Not specified';
    ...
    const noStudents = Array.isArray(context.noStudents) ? context.noStudents : [];
    const security = Array.isArray(context.security) ? context.security : [];
    const support = context.support ?? 'Not specified';
    const communicationMethod = context.communicationMethod ?? 'Not specified';
    const mobility = context.mobility ?? 'Not specified';
    const sensoryConsideration = context.sensoryConsideration ?? 'Not specified';
    ...
    const medicalEmergencyProtocol = context.medicalEmergencyProtocol ?? 'Not specified';
    const iepPlan = context.iepPlan ?? 'Not specified';
    const senOptions = Array.isArray(context.senOptions) ? context.senOptions : [];
    const cbtTest = context.cbtTest ?? 'No';
    const nationalTest = context.nationalTest ?? 'Not specified';
    const maxOptions = context.maxOptions ?? 'Not specified';
    const maxAnswers = context.maxAnswers ?? 1;
    ...
    const assessmentWeight = context.assessmentWeight ?? 'Not specified';
    const assessmentLearning = context.assessmentLearning ?? 'Not specified';
    const submissionFormat = Array.isArray(context.submissionFormat) ? context.submissionFormat : [];
    return `
