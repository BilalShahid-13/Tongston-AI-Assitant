...
exports.subjectAssessmentPrompt = subjectAssessmentPrompt;
exports.studentConductCharacterPlanPrompt = studentConductCharacterPlanPrompt;
...
${knowledgeBase}
You are tasked with generating **Scaffolded Subject Lesson Objectives** by referencing the **Fixed Subject Lesson Objectives** linked to each **Subject** under each **Discipline** for each **Week**, based on the user-selected **Term**.

---

## 🔹 Context
- **Year/Class**: ${context.yearClass}
- **Students’ Average-Age**: ${context.averageAge}
- **Week**: ${context.week}
- **Term**: ${context.term}
- **Term Theme**: ${context.termTheme}
- **Discipline**: ${context.discipline}
- **Subject**: ${context.subject}
- **Subject Unit/Topic**: ${context.unitTopic}
- **Fixed Subject Lesson Objectives**:
${context.fixedObjectives}

---

## 🔹 Development & Growth Realms (based on Term)
- Term 1 → *Personal Development & Growth* Objectives only
- Term 2 → *Professional Development & Growth* Objectives only
- Term 3 → *Public Development & Growth* Objectives only

---

## 🔹 Generation Rules
1. Each generated scaffolded objective must explicitly contribute to at least one **Entrepreneurial Thinking KPI** (VALUE, INFLUENCE, PROFITABILITY) from [LOSPPS_Ref].
2. Ensure alignment with **Entrepreneurial-Thinking** (Building Value, Influence, and Profitability) and **Socio-Economic Development & Growth**.
3. For each development realm, generate scaffolded objectives across all **4 levels of Bloom’s Taxonomy** [BT_Ref]:
   - Recall & Understanding
   - Analysis & Evaluation
   - Application & Synthesis
   - Metacognition & Transfer
4. Use **Students’ Average-Age-appropriate** language, pacing, and cognitive expectations for each level.
5. Ensure that each scaffolded objective is **additive** to the selected Fixed Subject Lesson Objective.
6. Extract the **Project Narrative** for the selected term theme and align objectives with it.

---

## 🔹 Selection Rules (Fixed Objectives)
1. User selects the **Discipline, Subject, and Week**.
2. System returns the **relevant Fixed Subject Lesson Objectives** for that selection.
3. Generation must stay aligned with these fixed objectives.
...
### PERSONAL DEVELOPMENT & GROWTH (if Term 1)
- Subject Lesson Description
- Objectives – Recall & Understanding
- Objectives – Analysis & Evaluation
- Objectives – Application & Synthesis
- Objectives – Metacognition & Transfer

### PROFESSIONAL DEVELOPMENT & GROWTH (if Term 2)
- Subject Lesson Description
- Objectives – Recall & Understanding
- Objectives – Analysis & Evaluation
- Objectives – Application & Synthesis
...
### PUBLIC DEVELOPMENT & GROWTH (if Term 3)
- Subject Lesson Description
- Objectives – Recall & Understanding
- Objectives – Analysis & Evaluation
- Objectives – Application & Synthesis
...
function subjectAssessmentPrompt(context) {
    return `
...
function studentConductCharacterPlanPrompt(context) {
    return `
...
//   `;
// }
function studentConductCharacterPlanAssessmentPrompt(context) {
    // Destructured variables remain the same (as in your version)...
    const typeofAssessments = context.typeofAssessments;
    const continuousAssessmentWeek = Array.isArray(context.continuousAssessmentWeek)
        ? context.continuousAssessmentWeek
        : [];
    const location = context.location;
    const yearClass = context.yearClass;
    const schoolLevel = context.schoolLevel;
    const term = context.term;
    const week = context.week;
    const KPI = context.KPI;
    const bloomLevel = context.bloomLevel;
    const technologyAccess = context.technologyAccess;
    const studentConductLessonObjectives = context.studentConductLessonObjectives;
    const subject = context.subject;
    const teachingAids = Array.isArray(context.teachingAids)
        ? context.teachingAids
        : [];
    const classSize = context.classSize;
    const timeAvailable = context.timeAvailable;
    const noQuestions = context.noQuestions;
    ...
    const classesSocioEconomic = context.classesSocioEconomic ?? getSocioEconomicContext(yearClass);
    const aim = context.aim ?? "Not specified";
    const termTheme = context.termTheme ??
        (term === "1"
            ...
            : term === "2"
                ...
                : "Public Development");
    const subjectLearning = context.subjectLearning ?? "Not specified";
    const sen = Array.isArray(context.sen) ? context.sen : [];
...
function projectTaskFacilitationPlan(context) {
    // Required fields
    const location = context.location || "Not specified";
    const yearClass = context.yearClass || "Not specified";
    const schoolLevel = context.schoolLevel || getSchoolLevel(yearClass);
    const term = context.term || "1";
    const task = context.task || "Not specified";
    const technologyAccess = context.technologyAccess || "No";
    const classSize = context.classSize || "10-25";
    const timeAvailable = context.timeAvailable || "40";
    const teachingAids = Array.isArray(context.teachingAids)
        ? context.teachingAids
        : [];
    // Optional fields
    ...
    const weeklyNotes = context.weeklyNotes || "No";
    const sen = Array.isArray(context.sen) ? context.sen : [];
    const noStudents = Array.isArray(context.noStudents)
        ? context.noStudents
        : [];
...
function projectTaskPlanPrompt(context) {
    // Required fields
    const location = context.location || "Not specified";
    const yearClass = context.yearClass || "Not specified";
    const schoolLevel = context.schoolLevel || getSchoolLevel(yearClass);
    ...
    const numberOfSubTasks = context.numberOfSubTasks || "1";
    const correctModel = context.correctModel || "No";
    const explanationCorrectModel = context.explanationCorrectModel || "No";
    const submissionFormat = Array.isArray(context.submissionFormat)
        ? context.submissionFormat
        : [];
    // Derive termly KPIs based on term
...
// Helper functions
function getSchoolLevel(yearClass) {
    const nursery = [
        "Nursery 1",
        "Nursery 2",
        "Nursery 3",
    ...
    if (nursery.includes(yearClass))
        return "Nursery School";
    if (primary.includes(yearClass))
        return "Primary School";
    if (secondary.includes(yearClass))
        return "Secondary School";
    if (tertiary.includes(yearClass))
        return "Tertiary School";
    return "Not specified";
}
function getSchoolSubLevel(yearClass) {
    const nursery = [
        "Nursery 1",
        "Nursery 2",
        "Nursery 3",
    ...
    if (nursery.includes(yearClass))
        return "Nursery School";
    if (lowerPrimary.includes(yearClass))
        return "Lower Primary School";
    if (upperPrimary.includes(yearClass))
        return "Upper Primary School";
    if (juniorSecondary.includes(yearClass))
        return "Junior Secondary School";
    if (seniorSecondary.includes(yearClass))
        return "Senior Secondary School";
    if (tertiary.includes(yearClass))
        return "Tertiary School";
    return "Not specified";
}
function getAverageAge(yearClass) {
    const ageMap = {
        "Nursery 1": "3–4 years",
...
function getSocioEconomicContext(yearClass) {
    const contextMap = {
        "Nursery 1": "House",
